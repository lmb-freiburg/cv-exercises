...
def photometric_loss(image_1, image_2, pred_flow, weight=None, alpha=0.5, eps=1e-3):
    ...
    # image_2_warped.shape = (N, 3, H, W)
    # warping_mask.shape = (N, 1, H, W) with 1 where warping was valid and 0 where it was not
    ...
    # photo_error.shape = (N, 3, H, W), pointwise_photo_loss.shape = (N, 3, H, W)

    # 4. Average the photometric error over all pixels where the warping was valid
    # (using the obtained warping mask).
    # We already set the invalid entries of pointwise_photo_loss to 0 above.
    # To average the loss correctly we need to divide by the number of valid pixels.
