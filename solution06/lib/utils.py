...
def project_to_image(X_ref, K, to_ref_transform, return_hom=False):
    ...
    # START TODO #################
    # Outline:
    # 1. Compute the camera matrix P from K and to_ref_transform. See camera calibration lecture.
...
def compute_essential_matrix(to_ref_transform):
    ...
    It maps a point in the reference camera to the corresponding epipolar line in the current
    camera, in homogeneous coordinates. It uses normalized image coordinates instead of pixels.
    The relation between essential matrix, fundamentral matrix and camera internal parameters is
    E = K^T F K
    ...
    Returns:
        Essential matrix from reference camera to current camera, shape (3, 3).
    """
    # START TODO #################
    # Outline:
...
def compute_fundamental_matrix(K, to_ref_transform):
    ...
    It maps an image point in one camera to the corresponding epipolar line in the other camera,
    ...
    Returns:
        Fundamental matrix from reference camera to current camera, shape (3, 3).
    """
    # START TODO #################
    # Outline:
