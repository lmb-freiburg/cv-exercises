...
class DispNet(nn.Module):
    def __init__(self, C=64, corr=True):
        ...
        if self.corr:
            self.corr_block = Corr(
                steps=40, step_size=1, cuda_corr=False, corr_type="disp"
            )
        ...
        self.deconv_5 = deconv(
            3 * C_last + 1, C_curr
        )  # input: [deconv4, conv2a, interp(pred_3)]
        self.pred_5 = pred_block(
            C_curr + C_last + 1
        )  # input: [deconv5, conv1a, interp(pred_4)]
    ...
    def forward(self, image_left, image_right):
        ...
        pred = F.interpolate(
            pred, size=deconv_5.shape[-2:], mode="bilinear", align_corners=False
        ).detach()
    ...
    def visualize_corr(self, image_left, image_right, x_refs=None, y_refs=None):
        ...
        return self.corr_block.visualize(
            image_left, image_right, conv3a, conv3b, x_refs=x_refs, y_refs=y_refs
        )
